...
const generateNewHostPort =
  (actual: HostContainer<string>) =>
  (UsedPorts?: HostContainer<string>[]): port => {
    const usedHostPortNumber = UsedPorts?.map((p) => parseInt(p.host)).filter(
      (p) => !isNaN(p)
    );
    ...
    let newHostPort = isAnIp
      ? parseInt(splitHost[splitHost.length - 1])
      : parseInt(actual.host);
