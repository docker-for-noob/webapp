...
const hostPortMustBeUnique = (usedPort?: HostContainer<string>[]): Validator =>
    (value: port) => {
        if (usedPort) {
...
const EnvKeyMustBeRelateToValue = (env: env): Validator => () => {
    if (env.key == "" && env.value != "") return keyMustBeInput;
    return undefined;
};

const EnvValueMustBeRelateToKey = (env: env): Validator => () => {
    if (env.key != "" && env.value == "") return valueMustBeInput;
    return undefined;
};

const VolumeHostMustBeRelateToContainer= (volumes: volumes): Validator => () => {
    if (volumes.container != "" && volumes.host == "") return hostPathMustBeInput;
    return undefined;
};

const VolumeContainerMustBeRelateToHost= (volumes: volumes): Validator => () => {
    if (volumes.container == "" && volumes.host != "") return containerPathMustBeInput;
    return undefined
};
