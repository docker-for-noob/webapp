...
const formatIEToStringArray = <T>(data?: InternalExternal<T>[]): string[] | undefined => data?.map(formatIEToString<T>)
const formatIEToString = <T>(e: InternalExternal<T>): string => [e.internal, e.external].join(":")

const formatEnvVarToKVPObject = (data: DockerContainer): any => data[Env] && {
