...
import {dataToDownload, fileName} from "../../domain/imageReference/service/downloader/type";
...
const inBrowser = async (filename: fileName, data: dataToDownload, option) => {
    ...
    if (blob.type === "unknown") return error<InfrastructureException>(BLOB_IS_NOT_A_VALID_TYPE(filename));
